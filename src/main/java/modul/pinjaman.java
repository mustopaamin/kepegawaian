...
public class pinjaman extends javax.swing.JFrame {
    ...
    public void datatables(){
        Object header[]={"No","NIP","Nama","Tanggal","Nominal","Keterangan"};
        DefaultTableModel data = new DefaultTableModel(null,header);
        tabelPinjaman.setModel(data);

        tabelPinjaman.getColumnModel().getColumn(0).setPreferredWidth(15);  // No
        tabelPinjaman.getColumnModel().getColumn(1).setPreferredWidth(20);  // NIP
        tabelPinjaman.getColumnModel().getColumn(4).setPreferredWidth(50);  // MASUK
        tabelPinjaman.getColumnModel().getColumn(5).setPreferredWidth(150);  // LULUS

        String sql = "SELECT a.nip,b.nama_lengkap,a.tanggal,a.nominal,a.keterangan FROM data_pinjaman a join data_pegawai b on b.nip=a.nip order by a.id desc";
        try {
            ...
            while (rs.next())
            {
                ...
                String kolom5 = rs.getString("nominal");
    ...
    private void simpan(){
        try{
	    String sql = "INSERT INTO data_pinjaman (nip, tanggal, nominal, keterangan) VALUES (?, ?, ?, ?)";
	    ...
	    stmt.setString(1, parts[0]);
            ...
            stmt.setString(3, nominal.getText());
	    ...
	    if (rowsInserted > 0) {
	        JOptionPane.showMessageDialog(null, "Simpan Pinjaman Berhasil", "", JOptionPane.INFORMATION_MESSAGE);
	    } else  {
                JOptionPane.showMessageDialog(null, "Simpan Pinjaman Gagal", "", JOptionPane.WARNING_MESSAGE);
            }
        }  catch (Exception e) {
    ...
    private void edit(){
        String sql = "UPDATE data_pinjaman SET nominal = ?, keterangan = ? WHERE nip = ? and tanggal = ?";
	    ...
	    stmt.setString(1, nominal.getText());
    ...
    private void hapus(){
        String sql="delete from data_pinjaman WHERE nip = ? and tanggal = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private void initComponents() {
        ...
        jLabel5.setText("Menu Pinjaman Pegawai");
        ...
        tabelPinjaman.setFont(new java.awt.Font("Roboto", 0, 15)); // NOI18N
        ...
        if (tabelPinjaman.getColumnModel().getColumnCount() > 0) {
            tabelPinjaman.getColumnModel().getColumn(0).setPreferredWidth(25);
            tabelPinjaman.getColumnModel().getColumn(4).setPreferredWidth(30);
            tabelPinjaman.getColumnModel().getColumn(5).setPreferredWidth(30);
        }
...
//                siapIsi(false);
    ...
    private void btn_cariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cariActionPerformed
        ...
        if(text_cari.getText().isEmpty() ){
            ...
            String sql = "SELECT a.nip,b.nama_lengkap,a.tanggal,a.nominal,a.keterangan FROM data_pinjaman a join data_pegawai b on b.nip=a.nip where b.nama_lengkap like ?";
            try {
                PreparedStatement stmt = conn.prepareStatement(sql);
...
//            siapIsi(false);
    ...
    private void tabelPinjamanMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabelPinjamanMouseClicked
        // TODO add your handling code here:
        int baris = tabelPinjaman.getSelectedRow();
        String dp = tabelPinjaman.getModel().getValueAt(baris, 1).toString()+"-"+tabelPinjaman.getModel().getValueAt(baris, 2).toString();
        datapegawai.setSelectedItem(dp);

        SimpleDateFormat tglview = new SimpleDateFormat("yyyy-MM-dd");
        Date dateView = null;
        try {
            dateView = tglview.parse((String) tabelPinjaman.getValueAt(baris, 3));
        } catch (ParseException ex) {
                Logger.getLogger(pegawai.class.getName()).log(Level.SEVERE, null, ex);
            }        
        tgl.setDate(dateView);

        nominal.setText(tabelPinjaman.getModel().getValueAt(baris, 4).toString());
        gelar.setText(tabelPinjaman.getModel().getValueAt(baris, 5).toString());
